...
const BillsGenerateManagement = () => {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ...
      ...filteredBills.map(bill => [
        ...
        bill.items?.length || 0,
                 `₹${bill.totalAmount?.toFixed(2) || bill.totalBillAmount?.toFixed(2) || 0}`,
        bill.orderId || "N/A"
      ])
  ...
  const getSummaryStats = () => {
    ...
    const lockedOrders = getLockedOrders() ? getLockedOrders().length : 0;
         const totalAmount = filteredBills.reduce((sum, bill) => sum + (bill.totalAmount || bill.totalBillAmount || 0), 0);
      ...
      <div className="row mb-4">
          ...
          <div className="col-lg-3 col-md-6 mb-3">
            <div className="card border shadow-sm h-100 border-top border-4 border-primary">
              <div className="card-body text-center">
                ...
                <div>
                  <h4 className="fw-bold text-primary mb-1">{summaryStats.totalBills}</h4>
          ...
          <div className="col-lg-3 col-md-6 mb-3">
            <div className="card border shadow-sm h-100 border-top border-4 border-warning">
              <div className="card-body text-center">
                ...
                <div>
                  <h4 className="fw-bold text-warning mb-1">{summaryStats.totalOrders}</h4>
          ...
          <div className="col-lg-3 col-md-6 mb-3">
            <div className="card border shadow-sm h-100 border-top border-4 border-success">
              <div className="card-body text-center">
                ...
                <div>
                  <h4 className="fw-bold text-success mb-1">{summaryStats.unlockedOrders}</h4>
                  <p className="mb-0 text-muted">Unlocked Orders</p>
                </div>
              </div>
            </div>
          </div>
          <div className="col-lg-3 col-md-6 mb-3">
            <div className="card border shadow-sm h-100 border-top border-4 border-info">
              <div className="card-body text-center">
                ...
                <div>
                  <h4 className="fw-bold text-info mb-1">₹{summaryStats.totalAmount.toFixed(2)}</h4>
        ...
        {loadingOrders ? (
          ...
          <div className="text-center py-5">
            ...
            <h6 className="text-muted">No unlocked orders found</h6>
            <p className="text-muted">No unlocked orders found to generate bills.</p>
          </div>
        ) : (
          <div className="row">
            <SafeDataRender
                    ...
                    <div className="card-body d-flex flex-column">
                      ...
                      <div className="mb-3">
                        <div className="d-flex align-items-center mb-2">
                          ...
                          <span className="fw-medium">{order.distributorId?.distributorName || order.distributorId?.name || 'Unknown'}</span>
                        </div>
                        <div className="d-flex align-items-center mb-2">
        ...
        {loadingBills ? (
          <div className="text-center py-5">
           ...
           getSavedBills().length === 0 ? (
             <div className="text-center py-5">
               ...
               <h6 className="text-muted">No saved bills found</h6>
               <p className="text-muted">No saved bills found for the selected criteria.</p>
             </div>
           ) : viewMode === 'table' ? (
             // Table View
                   ...
                   {getSavedBills().filter(bill => bill && bill._id).map(bill => (
                     <SafeRender key={bill._id} fallback={<tr><td colSpan="5">Error rendering bill</td></tr>}>
                       <tr>
                         ...
                         <td>
                           <span className="badge bg-success fs-6">₹{bill.totalAmount?.toFixed(2) || bill.totalBillAmount?.toFixed(2) || '0.00'}</span>
                         </td>
                         <td>
                           <div className="d-flex align-items-center">
                       ...
                       <div className="card-body d-flex flex-column">
                         ...
                         <div className="mb-3">
                           <div className="d-flex align-items-center mb-2">
                             <i className="fas fa-truck text-muted me-2" style={{width: '16px'}}></i>
                             <span className="text-muted me-2">Distributor:</span>
                             <span className="fw-medium">{bill.distributorId?.distributorName || bill.distributorId?.name || 'Unknown'}</span>
                           </div>
                           <div className="d-flex align-items-center mb-2">
                             <i className="fas fa-calendar text-muted me-2" style={{width: '16px'}}></i>
                             <span className="text-muted me-2">Bill Date:</span>
                             <span className="fw-medium">{new Date(bill.billDate).toLocaleDateString()}</span>
                           </div>
                           <div className="d-flex align-items-center mb-2">
                             <i className="fas fa-boxes text-muted me-2" style={{width: '16px'}}></i>
                             <span className="text-muted me-2">Items:</span>
                             <span className="fw-medium">{bill.items?.length || 0}</span>
                           </div>
                           <div className="d-flex align-items-center mb-2">
                             <i className="fas fa-rupee-sign text-muted me-2" style={{width: '16px'}}></i>
                             <span className="text-muted me-2">Total Amount:</span>
                             <span className="fw-medium">₹{bill.totalAmount?.toFixed(2) || bill.totalBillAmount?.toFixed(2) || '0.00'}</span>
                           </div>
                         </div>
                       </div>
                     </div>
          ...
          getLockedOrders().length === 0 ? (
            <div className="text-center py-5">
              ...
              <h6 className="text-muted">No locked bills found</h6>
              <p className="text-muted">No locked bills found for the selected criteria.</p>
            </div>
          ) : viewMode === 'table' ? (
            // Table View
            <div className="table-responsive">
                    ...
                    <div className="card border shadow-sm h-100">
                      <div className="card-header bg-light">
                        <div className="d-flex align-items-center justify-content-between">
                          ...
                          <span className={`badge ${order.locked ? 'bg-success' : 'bg-info'}`}>
                            {order.locked ? 'Locked' : 'Delivered'}
                          </span>
                        </div>
                      </div>
